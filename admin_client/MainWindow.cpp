...
QWidget *MainWindow::buildPilePage()
{
    ...
    row->addWidget(addPileBtn);
    row->addWidget(editPileBtn);
    row->addWidget(delPileBtn);
...
void MainWindow::refreshPiles()
{
    ...
    for (int i = 0; i < piles.size(); ++i) {
        ...
        m_pileTable->setItem(i, 0, textItem(QString::number(p.id), p.id));
...
void MainWindow::onRestartPile()
{
    ...
    if (QMessageBox::question(this, QStringLiteral("远程重启"),
                              QStringLiteral("确认向电桩 %1 发送重启指令？").arg(code))
        != QMessageBox::Yes)
        return;
