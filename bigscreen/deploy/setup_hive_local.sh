...
SERVER_HADOOP_HOME="${SERVER_HADOOP_HOME:-/opt/hadoop}"
SERVER_HADOOP_CONF_DIR="${SERVER_HADOOP_CONF_DIR:-$RUNTIME_DIR/hadoop-conf}"
...
JAVA_HOME="${HIVE_JAVA_HOME:-/usr/lib/jvm/java-8-openjdk-amd64}"
...
[[ -x "$JAVA_HOME/bin/java" ]] || { echo "[ERROR] 缺少 Java 8u371+：$JAVA_HOME"; exit 2; }
[[ -x "$SERVER_HADOOP_HOME/bin/hadoop" ]] || { echo "[ERROR] 缺少 Hadoop 服务端：$SERVER_HADOOP_HOME"; exit 2; }
...
<configuration>
  <property><name>javax.jdo.option.ConnectionURL</name><value>jdbc:mysql://127.0.0.1:3306/${HIVE_DB_NAME}?createDatabaseIfNotExist=true&amp;useSSL=false&amp;allowPublicKeyRetrieval=true&amp;serverTimezone=Asia/Shanghai</value></property>
  <property><name>javax.jdo.option.ConnectionDriverName</name><value>com.mysql.cj.jdbc.Driver</value></property>
  <property><name>javax.jdo.option.ConnectionUserName</name><value>${HIVE_DB_USER}</value></property>
...
if ! mysql -N -u"$HIVE_DB_USER" -p"$HIVE_DB_PASSWORD" -h127.0.0.1 "$HIVE_DB_NAME" \
  ...
  echo "[6/6] 初始化 Hive Metastore schema"
